...
add_user(){
    echo -e "\033[37;41m给新用户起个名字，不能和已有用户重复，请使用英文字母或者数字，不要使用空格、符号\033[0m"
...
start_menu(){
    ...
    echo " 1000M外贸硬路由"
    echo "========================="
    echo "1. 第一步请执行：升级系统内核"
    echo "2. 第二步：安装wireguard"（请完成第一步升级重启以后）
    echo "3. CENTOS7升级wireguard"（可以随时更新，获取最新版）
    echo "4. CENTOS7卸载wireguard"
    echo "5. 显示客户端二维码(只能显示默认）"
    echo "6. 增加用户"
    echo "0. 退出脚本"
    echo
    read -p "请输入数字:" num
